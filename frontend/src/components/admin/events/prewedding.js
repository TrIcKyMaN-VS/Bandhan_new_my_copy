...
import Orderslist from '../preweddingorderlist';

function PreweddingDetails(props) {
const [preweddingids , setpreweddingids] = useState([])
const [formdata, setformdata] = useState([])
const [now, setnow] = useState(false)

async function viewDetails(orderId){
  axios.get(`api/adminuserlist/preweddingorder/orderId/${orderId}`).then((res) => {
 ...
 useEffect(() => {
  ...
  return (        
    ...
    <div className="row my-5">
      <h3 className="fs-4 mb-3">Prewedding Orders</h3>
      <div className="col">
