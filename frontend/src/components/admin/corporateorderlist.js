...
function Orderslist(props) {
    ...
    useEffect(()=>{
      axios
      .get(`api/corporateEvent/paymentDetails/${forms[0].orderId}`)
      .then((res) => {
        if (res.status === 200) {
      ...
      axios.get(`api/corporateEvent/corporateInfo/${forms[0].orderId}`).then((res) => {
    ...
    function cancelorder(value){
      if(value=== "Accepted"){
        axios.post(`api/eventInfo/corporateaccepted/${forms[0].orderId}`).then((res) => {
        });
      }
      if(value=== "Declined"){
        axios.post(`api/eventInfo/corporatedeclined/${forms[0].orderId}`).then((res) => {
        });
      }
      if(value=== "Refund"){
        axios.post(`api/eventInfo/corporaterefund/${forms[0].orderId}`,{refundamount}).then((res) => {
        });
      }
    }
    function dlt(){
      axios.delete(`api/eventInfo/corporatedlt/${forms[0].orderId}`)
    ...
    function updateEventDetails() {
      ...
      axios
        .post("/api/corporateEvent/updateInfos", { eventDatas })
        .then((res) => {
          // console.log(res);
          if (res.status===200) {
            alert("success")
    ...
    function updatePaymentDetails() {
      ...
      axios
        .post("/api/corporateEvent/updatePaymentDetails", { paymentUpdation })
        .then((res) => {
          // console.log(res);
          if (res.status === 200) {
            alert(res.data);
