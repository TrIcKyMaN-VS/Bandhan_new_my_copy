...
function Orderslist(props) {
    ...
    useEffect(()=>{
      axios
      .get(`api/familyFunction/paymentDetails/${forms[0].orderId}`)
      .then((res) => {
        if (res.status === 200) {
      ...
      axios.get(`api/adminuserlist/familyfunctionpointsvoucher/${forms[0].userId}`).then((res) => {
    ...
    function cancelorder(value){
      if(value=== "Accepted"){
        axios.post(`api/eventInfo/familyfunctionaccepted/${forms[0].orderId}`).then((res) => {
        });
      }
      if(value=== "Declined"){
        axios.post(`api/eventInfo/familyfunctiondeclined/${forms[0].orderId}`).then((res) => {
        });
      }
      if(value=== "Refund"){
        axios.post(`api/eventInfo/familyfunctionrefund/${forms[0].orderId}`,{refundamount}).then((res) => {
        });
      }
    }
    function dlt(){
      axios.delete(`api/eventInfo/familyfunctiondlt/${forms[0].orderId}`)
